			...
			<table class="table align-items-center table-flush">
				<thead class="thead">
					<tr>
						...
						<th scope="col" class="text-center">새글/전체글</th>
						<th scope="col" class="text-center">사용자 표시</th>
...
function adminUpdate(brd_id){
	$.ajax({
		async:false,
		type:"post",
		url:"${contextPath}/admin/A_boardAdminList",
		data:{"brd_id":brd_id},
...
function deleteBoard(brd_id){
	var str = "#allPostNum"+brd_id;
	var postNum = $(str).val();
	if ("${user.level}"==9) {
		if (confirm(brd_id+" 번 게시판을 삭제하시겠습니까?")) {
			if (postNum!=0) {
				alert("게시판에 남아있는 글을 삭제하여 주십시오.");
			} else {
				$.ajax({
					async:false,
					type:"post",
